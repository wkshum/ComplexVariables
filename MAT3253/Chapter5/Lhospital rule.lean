...
theorem complex_lhopital_zero_zero'
    ...
    Tendsto (fun z ↦ f z / g z) (𝓝[≠] z₀) (𝓝 (f' / g')) := by
      ...
      have h_div : Filter.Tendsto (fun t => (f (z₀ + t) - f z₀) / (g (z₀ + t) - g z₀)) (nhdsWithin 0 {0}ᶜ) (nhds (f' / g')) := by
        have h_div : Filter.Tendsto (fun t => (f (z₀ + t) - f z₀) / t / ((g (z₀ + t) - g z₀) / t)) (nhdsWithin 0 {0}ᶜ) (nhds (f' / g')) := by
          convert hf.div hg _ using 1;
          · ext; simp +decide [ div_eq_inv_mul ] ;
          · simp_all +decide [ HasZeroOrder ];
            contrapose! hg_zero_order; rw [ show deriv g z₀ = g' by exact ( HasDerivAt.deriv <| by rw [ hasDerivAt_iff_tendsto_slope_zero ] ; simpa [ div_eq_inv_mul, hg0 ] using hg ) ] ; aesop;
        refine' h_div.congr' ( by filter_upwards [ self_mem_nhdsWithin ] with t ht using by rw [ div_div_div_cancel_right₀ ht ] );
      rw [ Metric.tendsto_nhdsWithin_nhds ] at *;
      intro ε hε; rcases h_div ε hε with ⟨ δ, hδ, H ⟩ ; exact ⟨ δ, hδ, fun { x } hx₁ hx₂ => by simpa [ * ] using H ( sub_ne_zero_of_ne hx₁ ) <| by simpa [ dist_eq_norm ] using hx₂ ⟩ ;
...
def HasDerivAtInfinity (f : ℂ → ℂ) (f' : ℂ) :=
  Filter.Tendsto (fun z ↦ z * f z) (Filter.cocompact ℂ) (nhds f')
...
theorem complex_lhopital_infinity_infinity
    ...
    Filter.Tendsto (fun z ↦ f z / g z) (Filter.cocompact ℂ) 
       (nhds (f' / g')) := by
      -- We have f(z)/g(z) = (z*f(z)) / (z*g(z)) for z ≠ 0.
      suffices h_eq : Filter.Tendsto (fun z => (z * f z) / (z * g z)) 
        (Filter.cocompact ℂ) (nhds (f' / g')) by
        refine' h_eq.congr' _ ; filter_upwards [ ( Filter.mem_cocompact.mpr ⟨ Metric.closedBall 0 1, ProperSpace.isCompact_closedBall _ _, by aesop_cat ⟩ ) ] with x hx ; 
      ...
      have h_tendsto : Filter.Tendsto (fun z => z * f z) (Filter.cocompact ℂ) (nhds f') ∧ Filter.Tendsto 
          (fun z => z * g z) (Filter.cocompact ℂ) (nhds g') := by
